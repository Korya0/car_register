...
class GSheetsCarNumberRemoteDataSourceImpl implements CarNumberRemoteDataSource {
  static final GSheetsCarNumberRemoteDataSourceImpl _instance = GSheetsCarNumberRemoteDataSourceImpl._internal();
  ...
  Future<Result<void>> initialize() async {
    try {
      final jsonString = await rootBundle.loadString(AppConstants.googleSheetCredentials);
      _gsheets = GSheets(jsonString);
      final spreadsheet = await _gsheets!.spreadsheet(AppConstants.spreadsheetId);
  ...
  Future<Worksheet?> _getOrCreateWorksheet(Spreadsheet spreadsheet) async {
    try {
      ...
      final newWorksheet = await spreadsheet.addWorksheet(AppConstants.sheetName);
      await newWorksheet.values.insertRow(1, [AppConstants.columnName, AppConstants.dateColumnName]);
      return newWorksheet;
    } catch (_) {
      try {
        final newWorksheet = await spreadsheet.addWorksheet(AppConstants.sheetName);
  ...
  Future<Result<List<CarNumberModel>>> _refreshCache() async {
    try {
      ...
      if (rows.isEmpty || rows.length <= 1) {
        ...
        _cachedNumbers = rows.skip(1).where((row) => row.isNotEmpty).map((row) {
          final number = row.first.toString().trim();
          final createdAtStr = row.length > 1 ? row[1].toString().trim() : '';
          DateTime? createdAt;
          if (createdAtStr.isNotEmpty) {
            createdAt = DateTime.tryParse(createdAtStr);
          }
          return CarNumberModel(number: number, createdAt: createdAt);
        }).where((model) => model.number.isNotEmpty).toList();
      }
  ...
  Future<Result<bool>> addNumber(CarNumberModel number) async {
    if (_worksheet == null || number.number.trim().isEmpty) return const Result.success(false);
    try {
      final trimmedNumber = number.number.trim();
      if (_cachedNumbers?.any((model) => model.number == trimmedNumber) == true) {
        return const Result.success(false);
      }
      
      await _worksheet!.values.appendRow([trimmedNumber, number.createdAt.toIso8601String()]);
  ...
  Future<Result<bool>> deleteNumber(String number) async {
    if (_worksheet == null || number.trim().isEmpty) return const Result.success(false);
    try {
      final trimmedNumber = number.trim();
      final rows = await _worksheet!.values.allRows();
      for (int i = rows.length - 1; i >= 1; i--) {
        if (rows[i].isNotEmpty && rows[i].first.toString().trim() == trimmedNumber) {
  ...
  Future<Result<bool>> deleteNumbers(List<String> numbers) async {
    if (_worksheet == null || numbers.isEmpty) return const Result.success(false);
    try {
  ...
  Future<Result<bool>> numberExists(String number) async {
    try {
      ...
      if (_isCacheValid() && _cachedNumbers != null) {
        return Result.success(_cachedNumbers!.any((model) => model.number == trimmedNumber));
      }
      final result = await getAllNumbers();
      return result.when(
        success: (models) => Result.success(models.any((model) => model.number == trimmedNumber)),
